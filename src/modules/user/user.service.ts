...
import { CompleteOnboardingDto } from '../auth/dto/create-vendor.dto';
...
export class UserService {
  ...
  async completeOnboarding(
    ...
    vendor.status = VendorStatus.PENDING_DOCUMENTS;
  ...
  private async checkAndUpdateOnboardingStatus(
    ...
    if (status.isComplete) {
      const vendor = await this.userRepository.findById(vendorId);
      if (vendor.status === VendorStatus.PENDING_DOCUMENTS) {
        vendor.status = VendorStatus.READY_FOR_REVIEW;
  ...
  private getDocumentName(documentType: DocumentType): string {
    const names = {
      ...
      [DocumentType.TAX_CERTIFICATE]: 'Tax Registration Certificate',
      [DocumentType.IDENTIFICATION]: 'Owner Identification',
  ...
  private getDocumentDescription(documentType: DocumentType): string {
    const descriptions = {
      ...
      [DocumentType.TAX_CERTIFICATE]: 'Tax registration or VAT certificate',
      [DocumentType.IDENTIFICATION]:
        "Government-issued ID (Passport, Driver's License)",
      [DocumentType.BANK_STATEMENT]:
        'Recent bank statement showing business name',
      [DocumentType.UTILITY_BILL]:
        'Utility bill for business address verification',
      [DocumentType.OTHER]: 'Additional supporting documents',
    };
    return descriptions[documentType];
  }

  private async notifyAdminForReview(vendor: Vendor): Promise<void> {
