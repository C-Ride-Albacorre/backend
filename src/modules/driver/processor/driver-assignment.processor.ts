...
export class DriverAssignmentProcessor extends WorkerHost {
  ...
  private async removeEtaScheduler(schedulerName: string): Promise<boolean> {
    try {
      ...
      this.driverAssignmentService.logger.warn(
        `⚠️ removeJobScheduler failed for ${schedulerName}: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
    ...
    try {
      ...
      this.driverAssignmentService.logger.warn(
        `⚠️ removeRepeatable fallback failed for ${schedulerName}: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
