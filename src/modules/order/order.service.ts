...
export class OrderService {
  ...
  // 1. Fetch order with store, items, driver assignment and rating
  const order = await this.prisma.order.findUnique({
    where: { id: orderId },
    include: {
      items: {
  ...
  // 5. Build driver & assignment data
  let driverData = undefined;
  let assignmentData = null;
  let driverId: string | null = null;
  ...
  if (assignment?.driverId) {
    ...
    const [driverUser, driverProfile, rating] = await Promise.all([
      this.prisma.user.findUnique({
        where: { id: driverId },
        select: {
          id: true,
      ...
      // Get the rating/comment for this order from this reviewer
    ...
    if (driverUser) {
      driverData = {
        id: driverUser.id,
        fullName: `${driverUser.firstName} ${driverUser.lastName}`.trim(),
        photo: driverUser.profilePicture,
        phone: driverUser.phoneNumber,
        ...
        // Rating information for this order
        review: {
          id: rating?.id ?? null,
          rating: rating?.rating ?? null,
          comment: rating?.comment ?? null,
          status: rating?.status ?? null,
          requestedAt: rating?.requestedAt ?? null,
          submittedAt: rating?.submittedAt ?? null,
          expiresAt: rating?.expiresAt ?? null,
        },
      };
    }
  ...
  try {
    ...
    this.logger.warn(
      `Failed to fetch Redis data for order ${orderId}: ${error}`,
    );
  }
